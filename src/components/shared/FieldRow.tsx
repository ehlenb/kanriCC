...
export function FieldRow({ label, children, className, highlight }: FieldRowProps) {
  const highlightClass =
    highlight === "warning"
      ? "bg-warning-bg rounded px-1 -mx-1"
      : highlight === "danger"
        ? "bg-danger-bg rounded px-1 -mx-1"
        : highlight === "success"
          ? "bg-success-bg rounded px-1 -mx-1"
          : "";

  return (
    <div
      className={cn(
        "flex justify-between items-start text-[13px] py-1.5 border-b border-border last:border-0 gap-3",
        highlightClass,
        className,
      )}
    >
